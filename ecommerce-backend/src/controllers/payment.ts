...
export const newcode = tryCatch(
    async (req, res, next) => {

        const { code, amount } = req.body;

        if (!code || !amount) return next(new errorHandler("please enter both the field", 400))
...
export const applyDsicount = tryCatch(
    async (req, res, next) => {
        ...
        if (!code) return next(new errorHandler("Invalid code", 400))
...
export const deleteCoupon = tryCatch(
    async (req, res, next) => {
        ...
        if (!id) return next(new errorHandler("please give id", 400))
